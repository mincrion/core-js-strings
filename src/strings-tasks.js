...
function removeFirstOccurrences(str, value) {
  const subPos = str.indexOf(value);
  if (subPos < 0) return str;
  return str.slice(0, subPos) + str.slice(subPos + value.length);
}
...
function removeLastOccurrences(str, value) {
  const subPos = str.lastIndexOf(value);
  if (subPos < 0) return str;
  return str.slice(0, subPos) + str.slice(subPos + value.length);
}
...
function isPalindrome(str) {
  let tmpStr = str;
  tmpStr = tmpStr.replaceAll(/[^a-zA-Z]/g, '').toLowerCase();
  return tmpStr.split('').reverse().join('') === tmpStr;
}
...
function findLongestWord(sentence) {
  return sentence.split(' ').reduce((longestWord, word) => {
